...
#[tauri::command]
fn upload(wlan: &str, pwd: &str, location: &str) -> String {
    match do_setup_process(wlan, pwd, location) {
        ...
        if let SerialPortType::UsbPort(info) = port.port_type {
            if let Some(product) = info.product {
                if product.contains("Pico")
                    || product.contains("RP2")
                    || product.contains("MicroPython")
                {
...
fn upload_file(
    ...
    if output.contains("\x04>OK\x04\x04>OK\x04\x04>OK\x04\x04>") {
        ...
        return Err(format!("[!] Upload error:\n{}", output).into());
    }
...
fn do_setup_process(
    ...
    let server_ip = if stdout.contains("camerapi") {
        stdout
            ...
            .next()
            .and_then(|line| {
                line.strip_prefix("Pinging camerapi [")
    ...
    if server_ip == "not found" {
        return Err(
            "Server IP not found. Please ensure the server is reachable and try again.".into(),
        );
    } else {
